...
**StockAnalysis** 是一个基于 **LangGraph 多智能体工作流** 的股票分析平台,采用 **模块化单体架构**,支持多用户登录注册,每个用户的配置和数据完全隔离。

**核心特性**:
- TradingAgents 智能分析系统(基于 LangGraph 四阶段工作流)
...
TradingAgents 是基于 **LangGraph** 的多阶段 AI 股票分析系统,模拟专业投资分析团队的工作模式。
...
- **WorkflowScheduler** (`backend/modules/trading_agents/scheduler/workflow_scheduler.py`) - LangGraph 工作流调度器

...
- [ ] 2.1 实现 LLM Provider 抽象层 (llm/provider.py)
  - 定义 LLMProvider 抽象基类
...
- [ ] 2.2 实现 OpenAI 兼容适配器 (llm/openai_compat.py)
...
- [ ] 3.1 实现 MCP 适配器基类 (tools/mcp_adapter.py)
  - 定义 MCPAdapter 抽象基类
...
- [ ] 3.2 实现 stdio 模式 MCP 适配器
  - 实现进程启动和管理
  - 实现 JSON-RPC 通信
  - 实现超时和错误处理
  - 需求: 2.2

- [ ] 3.3 实现 SSE 模式 MCP 适配器
  - 实现 SSE 连接管理
  - 实现事件流解析
  - 实现认证处理
  - 需求: 2.3

- [ ] 3.4 实现 HTTP 模式 MCP 适配器
  - 实现 HTTP 请求封装
  - 实现认证处理（Bearer/Basic）
  - 实现响应解析
  - 需求: 2.3
...
- [ ] 4.1 实现工具注册表 (tools/registry.py)
  - 实现 ToolRegistry 单例类
  - 实现工具注册和查找方法
  - 实现工具可用性状态管理
  - 需求: 12.2, 12.3, 12.4

- [ ] 4.2 实现本地工具接口预留 (tools/local_tools.py)
  - 定义 LocalTool 抽象基类
  - 实现工具注册装饰器
  - 预留工具实现接口
  - 需求: 12.7
...
- [ ] 4.4 实现工具循环检测器 (tools/loop_detector.py)
...
- [ ] 5.1 实现健壮的并发控制器 (core/concurrency.py)
  - 实现基于 Redis 的分布式信号量
...
- [ ] 5.4 实现 MCP 服务器并发控制 (tools/mcp_concurrency.py)
...
- [ ] 7.1 创建默认智能体配置模板 (config/templates/agents.yaml)
  - 定义第一阶段默认分析师配置（可配置多个）
  - 定义第二阶段研究员配置（增加 max_rounds 参数，默认 1）
  - 定义第三阶段风险分析师配置
  - 定义第四阶段总结智能体配置

- [ ] 7.2 实现第二阶段辩论逻辑 (agents/phase2/)
...
- [ ] 9.3 实现第一阶段动态分析师工厂 (agents/phase1/)
...
- [ ] 9.4 实现第二阶段智能体
  - 实现 BullResearcher 看涨研究员
  - 实现 BearResearcher 看跌研究员
  - 实现 ResearchManager 研究经理
  - 实现 Trader 交易员
  - 需求: 3.5, 13.1

- [ ] 9.5 实现第三阶段智能体
  - 实现 AggressiveAnalyst 激进派
  - 实现 ConservativeAnalyst 保守派
  - 实现 NeutralAnalyst 中性派
  - 实现 RiskManager 首席风控官
  - 需求: 3.5, 13.2

- [ ] 9.6 实现第四阶段智能体
  - 实现 SummaryAgent 总结智能体
  - 实现结构化报告生成（推荐结果+价格）
  - 实现流式输出支持
  - 需求: 4.7, 4.10
...
- [ ] 10.1 实现智能体执行引擎 (core/agent_engine.py)
  - 实现 AgentEngine 类
  - 实现 LangGraph 图构建
  - 实现条件路由逻辑
  - 实现阶段开关控制
...
- [ ] 11.1 实现 WebSocket 管理器 (websocket/manager.py)
  - 实现按需连接（用户打开详情页面时连接）
  - 实现连接管理（建立、断开）
  - 实现单用户最多5个连接限制
  - 实现事件广播
...
- [ ] 11.2 定义 WebSocket 事件 (websocket/events.py)
  - 定义所有事件类型
  - 实现事件序列化
...
- [ ] 13.1 实现任务管理器 (core/task_manager.py)
  - 实现 create_task 方法
...
- [ ] 13.6 实现任务过期机制 (core/task_expiry.py)

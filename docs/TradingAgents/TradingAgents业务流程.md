...
**核心特性 - 完全动态化**:
- **无硬编码智能体**: Phase 1 的所有分析师节点都是动态创建的，从用户配置加载
- **用户完全可控**: 用户可以添加、删除、修改 Phase 1 的分析师智能体
- **配置驱动**: 智能体的数量、名称、角色定义(role_definition)、工具配置全部来自配置文件
- **只传递最终报告**: Phase 1 只将最终的分析报告传递给后续阶段,不传递中间过程

**业务流程**:
1.  **动态创建分析师节点**:
    *   系统从用户配置(或默认配置)读取 Phase 1 的分析师列表
    *   对每个启用的分析师,使用 `create_phase1_node_factory` 工厂函数动态创建节点
    *   默认配置包括4个分析师:市场技术分析师、情绪分析师、基本面分析师、新闻分析师
    *   用户可以自定义添加新的分析师,或修改/删除现有的分析师
...
4.  **使用配置中的提示词**: 每个分析师使用配置文件中定义的 `role_definition` 作为提示词,不再有硬编码。
5.  **生成初步报告**: 每个节点调用大语言模型（LLM）生成一份聚焦于自己领域的初步分析报告。
6.  **状态累积**: 使用 LangGraph 的 Reducer 模式，自动将各节点的报告累积到 `analyst_reports` 列表中。

**流程图**:
```mermaid
graph TD
    subgraph "Phase 1: 动态并行分析"
        P1_Start[START] --> P1_Config[加载用户配置];
        ...
        subgraph "并行执行（LangGraph 状态路由）"
            ...
            P1_A1 --> P1_A1_Tool[调用 MCP 工具];
            P1_A2 --> P1_A2_Tool[调用 MCP 工具];
            P1_A3 --> P1_A3_Tool[调用 MCP 工具];
            P1_An --> P1_An_Tool[调用 MCP 工具];

            P1_A1_Tool --> P1_A1_Report[生成最终报告];
            P1_A2_Tool --> P1_A2_Report[生成最终报告];
            P1_A3_Tool --> P1_A3_Report[生成最终报告];
            P1_An_Tool --> P1_An_Report[生成最终报告];
        end

        P1_A1_Report --> P1_Check{完成检查};
...
**LangGraph 技术实现**:
- **动态节点创建**: 使用 `create_phase1_node_factory` 工厂函数根据配置动态生成节点
- **并行路由**: 从 START 节点连接到所有动态创建的分析师节点
- **状态累积**: 使用 `operator.add` reducer 自动累积报告
- **完成检查**: 通过条件边检查 `completed_analysts` 计数,与 `expected_analysts` 对比
...
TradingAgents 完全集成项目核心 AI 系统，支持灵活的双模型配置。

### 5.1. 双模型架构

**设计理念**：根据不同阶段的计算特性，使用不同的模型以优化性能和成本。

| 模型类型 | 使用阶段 | 典型特征 | 推荐模型 |
|---------|---------|---------|---------|
| **数据收集模型** (data_collection_model) | Phase 1 | 频繁调用工具、处理大量数据、快速响应 | glm-4-flash（速度快、成本低） |
| **辩论模型** (debate_model) | Phase 2/3/4 | 复杂推理、多轮对话、深度分析 | glm-4-plus（能力强、推理好） |

**双模型配置示例**：
```python
model_config = {
    "data_collection_model": "glm-4-flash",  # Phase 1 使用快速模型
    "debate_model": "glm-4-plus",            # Phase 2/3/4 使用强模型
}
```
...
**优先级顺序**：
1. **任务参数**: 用户创建任务时明确指定的 model_config
2. **用户偏好**: 用户在 AI 设置中配置的默认模型
3. **系统默认**: 系统公共配置中的默认模型
...
if not model_config_obj.api_key_valid:
    raise ValueError(f"模型 API Key 无效: {model_id}")
```

### 5.3. LLM Provider 缓存

**缓存策略**：进程级别缓存，避免重复创建 LLM Provider 实例。

**缓存键**：`(user_id, model_id)`
- 相同用户 + 相同模型 = 共享同一个 LLMProvider 实例
- 不同模型 = 创建不同的 LLMProvider 实例

**缓存效果**：
```python
# 第一次调用：创建新实例
llm1 = await _get_llm_for_agent(state, "market_technical", phase="phase1")
# [LLM] 创建 LLM Provider: model=glm-4-flash, phase=phase1, agent=market_technical

# 第二次调用（同一模型）：使用缓存
llm2 = await _get_llm_for_agent(state, "fundamentals", phase="phase1")
# [LLM] 缓存命中: model=glm-4-flash, agent=fundamentals
```

**缓存管理**：
- 自动缓存：首次调用时自动缓存
- 手动清除：`clear_llm_cache()` 用于测试或内存管理
...
TradingAgents 节点
    ↓
_get_llm_for_agent(agent_slug, phase)
    ↓
┌─────────────────────────────────────┐
│ 1. 选择模型 ID                      │
│    - phase="phase1" → data_collection_model │
│    - phase="phase2" → debate_model    │
└─────────────────────────────────────┘
    ↓
┌─────────────────────────────────────┐
│ 2. 检查缓存                          │
...
**_get_llm_for_agent 函数**（`backend/modules/trading_agents/workflow/nodes.py`）：
```python
async def _get_llm_for_agent(
    ...
    Args:
        state: 工作流状态
        agent_slug: 智能体标识符
        phase: 阶段标识（"phase1" 使用 data_collection_model，
                       "phase2" 使用 debate_model）
...
**关键特性**：
- ✅ 完全集成项目核心 AIModelService
- ✅ 支持双模型配置（数据收集 vs 辩论）
- ✅ 进程级缓存优化性能
- ✅ 完善的错误处理和回退机制
- ✅ 模型验证（启用状态、API Key 有效性）
- ✅ 异步设计，符合 LangGraph 最佳实践
...
- LLM 响应缓存（相同输入直接返回）
- 工具调用结果缓存
- **LLM Provider 缓存**：进程级缓存，避免重复创建实例
...
2. **Phase 1 完全动态化**:
    *   **无硬编码**: 所有 Phase 1 智能体从配置动态创建
    *   **用户可控**: 支持添加、删除、修改 Phase 1 智能体
    *   **配置驱动**: 角色定义、工具配置全部来自配置文件
3. **AI 系统集成**: 完全集成项目核心 AIModelService，支持双模型配置
4. **并行执行**: 关键阶段并行化，提升效率
5. **可观测性**: 完整的日志、Token 追踪、进度推送
6. **容错性**: 多层次的错误处理和恢复机制
7. **可扩展性**: Phase 1 智能体数量不限,用户可灵活自定义

**关键设计原则**:
- **Phase 1**: 完全动态,用户可自定义(添加/删除/修改)
- **Phase 2, 3, 4**: 结构固定,但提示词可配置
- **数据流**: 只传递最终报告,不传递中间过程
- **AI 配置**: 使用项目核心 AI 系统，支持双模型和 LLM Provider 缓存

**技术栈**:
- **工作流引擎**: LangGraph (StateGraph)
- **LLM**: 智谱 AI glm-4.6（可通过项目核心 AI 系统配置）
- **AI 集成**: AIModelService + LLMProviderFactory

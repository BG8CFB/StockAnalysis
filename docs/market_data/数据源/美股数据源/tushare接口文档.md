...
> **适用市场**: 美股
> **测试状态**: 已于 2026-01-03 完成接口测试（2/6 核心接口可用，33.3%）
...
| 接口 | 状态 | 积分要求 | 说明 |
|------|------|----------|------|
| `us_basic()` | ✅ 可用 | 120 试用 / 5,000 正式 | 成功获取 6,000 条美股基础信息 |
| `us_daily()` | ✅ 可用 | 120 试用 / 5,000 正式 | 成功获取 21 条日线数据 |
| `us_income()` | ⚠️ 需要积分 | **15,000** | 测试积分不足，接口需要 15,000 积分 |
| `us_balancesheet()` | ⚠️ 需要积分 | **15,000** | 测试积分不足，接口需要 15,000 积分 |
| `us_cashflow()` | ⚠️ 需要积分 | **15,000** | 测试积分不足，接口需要 15,000 积分 |
| `us_fina_indicator()` | ⚠️ 需要积分 | **15,000** | 测试积分不足，接口需要 15,000 积分 |

**说明**：
1. ~~**us_mins**~~：已确认 TuShare 不提供美股分钟数据接口，已从文档删除
2. **财务数据接口** (`us_income`, `us_balancesheet`, `us_cashflow`, `us_fina_indicator`)：需要 **15,000 积分**，测试使用 5,000 积分的 KEY 无法访问
3. **测试 KEY 积分等级**：5,000 积分，可访问基础信息和日线行情，但不足以访问财务数据
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 财务数据返回空 | 积分不足（需要15,000积分） | 当前测试KEY为5,000积分，不足以访问美股财务数据 |
...
2. **美股分钟数据**
   - TuShare 不提供美股分钟数据
   - 建议使用日线数据或其他数据源（如 Yahoo Finance）

3. **美股财务数据**
   - 需要 **15,000 积分**才能获取
   - 5,000 积分的 KEY 无法访问这些接口
   - 积分不足时接口会返回空数据

4. **积分等级说明**
   - 120 积分：可试用 `us_basic` 和 `us_daily`
   - 5,000 积分：可正常访问基础信息和日线行情
   - 15,000 积分：可访问美股财务数据（利润表、资产负债表、现金流量表、财务指标）

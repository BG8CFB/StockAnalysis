...
**注意**: 基础积分（2000分）下，此接口每天最多调用 10 次。如需更高频次，请使用 `pro_bar` 接口。
...
| 参数名 | 类型 | 必选 | 说明 |
|-------|------|------|------|
| ts_code | str | Y | 港股股票代码（如 00700.HK） |
| start_date | str | N | 开始日期 |
| end_date | str | N | 结束日期 |
...
**推荐**: 使用 `pro_bar` 接口获取港股行情数据，无调用次数限制。
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| hk_daily 调用失败 | 权限不足 | 使用 `pro_bar` 接口替代 |
| 接口名错误 | 文档与实际不一致 | 使用 `hk_tradecal` 而非 `hk_trade_cal` |

### 7.2 最佳实践

1. **推荐使用 `pro_bar` 接口**获取港股日线数据
   - 无调用次数限制
   - 支持复权
   - 支持多种周期（日/周/月）

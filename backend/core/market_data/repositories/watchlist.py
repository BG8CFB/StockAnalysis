...
class UserWatchlistRepository:
    ...
    async def get_user_watchlist(self, user_id: str) -> Optional[Dict[str, Any]]:
        """获取用户的自选股列表"""
        result: Optional[Dict[str, Any]] = await self.collection.find_one({"user_id": user_id})
        return result

    async def add_stock(
        self, user_id: str, symbol: str, market: str, notes: Optional[str] = None
    ) -> None:
        """添加股票到自选"""
        # 检查是否已存在
        existing = await self.collection.find_one({"user_id": user_id, "stocks.symbol": symbol})
        ...
        if existing:
            ...
            await self.collection.update_one(
                ...
                {"$set": {"stocks.$": stock_data, "updated_at": datetime.now()}},
            )
        else:
            # 添加新股票
            await self.collection.update_one(
                {"user_id": user_id},
                {"$push": {"stocks": stock_data}, "$set": {"updated_at": datetime.now()}},
                upsert=True,
            )
    ...
    async def remove_stock(self, user_id: str, symbol: str) -> bool:
        ...
        result = await self.collection.update_one(
            {"user_id": user_id},
            {"$pull": {"stocks": {"symbol": symbol}}, "$set": {"updated_at": datetime.now()}},
        )

...
class StockFinancialRepository(BaseRepository):
    ...
    async def upsert_many(self, financials: List[StockFinancial]) -> List[int]:
        """
        批量新增或更新财务数据
        ...
        for financial in financials:
            count = await self.upsert_financial(financial)
            results.append(count)
        return results

    async def get_financial(
        self, symbol: str, report_date: str, report_type: str
        ...
        Returns:
            财务数据，未找到返回None
        """
        filter_query = {"symbol": symbol, "report_date": report_date, "report_type": report_type}
        return await self.find_one(filter_query)

    async def get_latest_financial(
        self, symbol: str, report_type: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        """
        获取最新的财务数据
        ...
        results = await self.find_many(filter_query, sort=[("report_date", -1)], limit=1)
    ...
    async def get_financial_history(
        ...
        return await self.find_many(filter_query, sort=[("report_date", -1)], limit=limit)
    ...
    async def get_financials_by_symbols(
        ...
        Returns:
            财务数据列表
        """
        filter_query: Dict[str, Any] = {"symbol": {"$in": symbols}}
        if report_date:
    ...
    async def get_income_statement(self, symbol: str, report_date: str) -> Optional[Dict[str, Any]]:
        """
        获取利润表数据
        ...
        Returns:
            利润表数据
        """
        filter_query = {"symbol": symbol, "report_date": report_date}
    ...
    async def get_balance_sheet(self, symbol: str, report_date: str) -> Optional[Dict[str, Any]]:
        """
        获取资产负债表数据
    ...
    async def get_cash_flow(self, symbol: str, report_date: str) -> Optional[Dict[str, Any]]:
        """
        获取现金流量表数据
    ...
    async def delete_by_symbol(self, symbol: str, before_date: Optional[str] = None) -> int:
        """
        删除指定股票的财务数据
    ...
    async def get_financial_count(
        self, symbol: Optional[str] = None, market: Optional[MarketType] = None
    ) -> int:
        """
        统计财务数据数量
...
class StockFinancialIndicatorRepository(BaseRepository):
    ...
    async def init_indexes(self) -> None:
        ...
        await self.create_index([("end_date", -1)])
        await self.create_index([("market", 1)])
        await self.create_index([("roe", -1)])
        await self.create_index([("net_margin", -1)])
    ...
    async def upsert_many(self, indicators: List[StockFinancialIndicator]) -> List[int]:
        """
        批量新增或更新财务指标数据
        ...
        for indicator in indicators:
            count = await self.upsert_indicator(indicator)
            results.append(count)
        return results

    async def get_indicator(self, symbol: str, end_date: str) -> Optional[Dict[str, Any]]:
        """
        获取指定股票和报告期的财务指标
        ...
        Returns:
            财务指标数据，未找到返回None
        """
        filter_query = {"symbol": symbol, "end_date": end_date}
        return await self.find_one(filter_query)

    async def get_latest_indicator(self, symbol: str) -> Optional[Dict[str, Any]]:
        """
        获取最新的财务指标
        ...
        """
        filter_query: Dict[str, Any] = {"symbol": symbol}
        results = await self.find_many(filter_query, sort=[("end_date", -1)], limit=1)
        return results[0] if results else None
    ...
    async def get_indicator_history(
        ...
        filter_query: Dict[str, Any] = {"symbol": symbol}
        if start_date:
            filter_query["end_date"] = {"$gte": start_date}
        if end_date:
            if "end_date" not in filter_query:
                filter_query["end_date"] = {}
            filter_query["end_date"]["$lte"] = end_date

        return await self.find_many(filter_query, sort=[("end_date", -1)], limit=limit)
    ...
    async def get_indicators_by_symbols(
        ...
        filter_query: Dict[str, Any] = {"symbol": {"$in": symbols}}
        if end_date:
            filter_query["end_date"] = end_date
    ...
    async def search_by_roe(
        ...
        Returns:
            符合条件的股票指标列表
        """
        filter_query: Dict[str, Any] = {"roe": {"$gte": min_roe}}
        if max_roe is not None:
            filter_query["roe"]["$lte"] = max_roe
        if end_date:
            filter_query["end_date"] = end_date

        return await self.find_many(filter_query, sort=[("roe", -1)], limit=limit)

    async def search_by_net_margin(
        self,
        min_margin: float,
        ...
        Returns:
            符合条件的股票指标列表
        """
        filter_query: Dict[str, Any] = {"net_margin": {"$gte": min_margin}}
        if max_margin is not None:
            filter_query["net_margin"]["$lte"] = max_margin
        if end_date:
            filter_query["end_date"] = end_date

        return await self.find_many(filter_query, sort=[("net_margin", -1)], limit=limit)
    ...
    async def get_financial_summary(self, symbol: str) -> Dict[str, Any]:
        ...
        pipeline: List[Dict[str, Any]] = [
            {"$match": {"symbol": symbol}},
            {
                "$group": {
                    "_id": "$symbol",
                    "latest_report": {"$first": "$end_date"},
                    "latest_roe": {"$first": "$roe"},
                    "latest_roa": {"$first": "$roa"},
                    "latest_net_margin": {"$first": "$net_margin"},
                    "latest_gross_margin": {"$first": "$gross_margin"},
    ...
    async def delete_by_symbol(self, symbol: str, before_date: Optional[str] = None) -> int:
        ...
        filter_query: Dict[str, Any] = {"symbol": symbol}
        if before_date:
            filter_query["end_date"] = {"$lt": before_date}

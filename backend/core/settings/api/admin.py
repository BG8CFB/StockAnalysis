...
from core.auth.rbac import Role, check_role
...
async def admin_update_trading_agents_settings(
    request: TradingAgentsSettingsUpdate, current_user: UserModel = Depends(get_current_active_user)
) -> Dict[str, Any]:
    """
...
async def admin_reset_trading_agents_settings(
    ...
    return {"message": "全局配置已重置为默认值", "settings": default_settings}

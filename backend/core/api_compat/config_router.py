...
async def update_datasource_config(
    ...
    update_fields: Dict[str, Any] = {}
    for field_name in [
        "type", "endpoint", "timeout", "rate_limit", "enabled",
        "priority", "config_params", "description", "market_categories",
...
async def update_database_config(
    ...
    update_fields: Dict[str, Any] = {}
    for field_name in [
        "type", "host", "port", "username", "database",
        "connection_params", "pool_size", "max_overflow", "enabled", "description",
    ]:
        val = getattr(data, field_name, None)
        if val is not None:
            update_fields[field_name] = val
...
async def export_config(
    ...
    # LLM 厂家
    providers = await db.llm_providers.find(
        {}, {"api_key": 0, "api_secret": 0}
    ).to_list(length=None)
    for p in providers:
        p.pop("_id", None)

...
from core.mcp.pool.connection import ConnectionState, MCPConnection
from core.mcp.schemas import MCPServerConfigResponse
...
class MCPConnectionPool:
    ...
    def _get_user_semaphore(self, server_id: str, user_id: str) -> asyncio.Semaphore:
        ...
        if user_id not in self._semaphores[server_id]:
            # 根据服务器类型动态读取并发限制（支持热更新）
            server_info = self._servers.get(server_id, {})
            is_system = server_info.get("is_system", False)

            if is_system:
                # 公共 MCP：每用户 10 并发（动态读取）
                limit = get_pool_public_per_user_max()
            else:
                # 个人 MCP：每用户 100 并发（动态读取）
                limit = get_pool_personal_max_concurrency()
    ...
    def reset_all_semaphores(self) -> int:
        """
        重置所有用户信号量（配置更新时调用）
        ...
        for server_id, user_semaphores in list(self._semaphores.items()):
            # 清空所有服务器的信号量
            count = len(user_semaphores)
            del self._semaphores[server_id]
            self._semaphores[server_id] = {}
            reset_count += count

        logger.info(
            f"[MCPConnectionPool] 重置所有信号量: {reset_count} 个 "
            f"（配置已更新，下次请求将使用新限制）"
        )

...
class MCPSystemSettingsBase(BaseModel):
    ...
    pool_personal_max_concurrency: int = Field(
        default=100, ge=1, le=1000, description="个人并发上限（每个用户的个人 MCP 最大并发连接数）"
    )
    ...
    pool_personal_queue_size: int = Field(
        default=200, ge=10, le=1000, description="个人队列大小（个人 MCP 请求队列最大容量）"
    )
    pool_public_queue_size: int = Field(
        default=50, ge=10, le=500, description="公共队列大小（公共 MCP 请求队列最大容量）"
    )

    # 连接生命周期
    connection_complete_timeout: int = Field(
        default=2, ge=1, le=300, description="完成超时时间（任务完成后连接销毁等待时间，秒）"
    )
    connection_failed_timeout: int = Field(
        default=10, ge=1, le=600, description="失败超时时间（任务失败后连接销毁等待时间，秒）"
    )

    # 健康检查
    health_check_enabled: bool = Field(default=True, description="是否启用自动健康检查")
    health_check_interval: int = Field(
        default=300, ge=10, le=3600, description="健康检查间隔时间（秒）"
    )
    health_check_timeout: int = Field(
        default=30, ge=5, le=300, description="单次健康检查超时时间（秒）"
    )

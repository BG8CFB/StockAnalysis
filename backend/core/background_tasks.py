...
async def create_analysis_task_background(
    user_id: str, request: Any, config: Dict[str, Any]
) -> str:
    """
    后台任务：创建并执行分析任务
    ...
    """
    logger.info(
        "[BackgroundTasks] ============ create_analysis_task_background CALLED ============"
    )
    ...
    # 委托给 task_manager 执行
    logger.info("[BackgroundTasks] calling task_manager.create_task_background...")
    task_id = await task_manager.create_task_background(
        user_id=user_id, request=request, config=config
    )
